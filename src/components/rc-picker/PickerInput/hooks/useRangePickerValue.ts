...
>(
  ...
  currentIndexPickerValue: DateType,
  ...
  const currentPickerValue = React.useMemo(() => {
    const current = [mergedStartPickerValue, mergedEndPickerValue][
      mergedActiveIndex
    ];
  ...
  useLayoutEffect(() => {
    if (open) {
      if (!defaultPickerValue[mergedActiveIndex]) {
        let nextPickerValue: DateType = isTimePicker
          ? null
        ...
        if (
          ...
          nextPickerValue = [mergedStartPickerValue, mergedEndPickerValue][
            mergedActiveIndex ^ 1
          ];
