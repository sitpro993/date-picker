...
function Picker<DateType extends object = any>(
  ...
  const internalMode: InternalMode =
    mergedMode === "date" && showTime ? "datetime" : mergedMode;
  ...
  const onSelectorClick: React.MouseEventHandler<HTMLDivElement> = (event) => {
    if (
      !disabled &&
      !selectorRef.current.nativeElement.contains(document.activeElement)
    ) {
  ...
  const [internalHoverValue, setInternalHoverValue] =
    React.useState<DateType>(null);

  const hoverValues = React.useMemo(() => {
    const values = [internalHoverValue, ...calendarValue].filter(
      (date) => date
    );
  ...
  const onPresetSubmit = (nextValue: DateType) => {
    const nextCalendarValues = multiple
      ? toggleDates(getCalendarValue(), nextValue)
      : [nextValue];
  ...
  const onPanelSelect = (date: DateType) => {
    ...
    const nextValues = multiple
      ? toggleDates(getCalendarValue(), date)
      : [date];
