...
function RangePicker<DateType extends object = any>(
  ...
  // ========================= Mode =========================
  const [modes, setModes] = useMergedState<[PanelMode, PanelMode]>(
    [picker, picker],
    {
      value: mode,
    }
  ...
  // >>> Calendar
  const onPanelSelect: PickerPanelProps<DateType>["onChange"] = (
    date: DateType
  ) => {
    lastOperation("panel");

    const clone: RangeValueType<DateType> = fillIndex(
      calendarValue,
      activeIndex,
      date
    );
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    if (
      disabled.some(
        (fieldDisabled, index) =>
          fieldDisabled && isIndexEmpty(index) && !allowEmpty[index]
      )
    ) {
      warning(
        false,
...
const RefRangePicker = React.forwardRef(RangePicker) as <
