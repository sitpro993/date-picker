...
const Input = React.forwardRef<InputRef, InputProps>((props, ref) => {
  ...
  const { prefixCls, input: Component = "input" } =
  ...
  const [forceSelectionSyncMark, forceSelectionSync] =
  ...
  const onInternalChange: React.ChangeEventHandler<HTMLInputElement> = (
    event
  ) => {
  ...
  const onFormatPaste: React.ClipboardEventHandler<HTMLInputElement> = (
    event
  ) => {
    // Get paste text
  ...
  const onFormatMouseUp: React.MouseEventHandler<HTMLInputElement> = (
    event
  ) => {
  ...
  // ======================= Keyboard =======================
  const onSharedKeyDown: React.KeyboardEventHandler<HTMLInputElement> = (
    event
  ) => {
  ...
  const onFormatKeyDown: React.KeyboardEventHandler<HTMLInputElement> = (
    event
  ) => {
    ...
    const offsetCellValue = (offset: number) => {
      ...
      if (isNaN(currentTextNum)) {
        return String(
          rangeDefault ? rangeDefault : offset > 0 ? rangeStart : rangeEnd
        );
      }
