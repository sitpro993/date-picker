...
export type NextActive<DateType> = (
  nextValue: RangeValueType<DateType>
) => number | null;
...
export default function useRangeActive<DateType>(
  ...
  const nextActiveIndex: NextActive<DateType> = (
    nextValue: RangeValueType<DateType>
  ) => {
    const list = activeListRef.current;
    const filledActiveSet = new Set(
      list.filter((index) => nextValue[index] || empty[index])
    );
